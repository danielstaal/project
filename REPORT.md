...
The decision to create a single screen to be able to view a global graph of the results and a single example at the same time seems to be a good decision. It reduces the complexity of the application by not having to switch screens or close any screens while examining the results. The decision to reduce the application from a solver to an application that just shows database results could be questionable. The option to enable the user to view live computation of the algorithm might stimulate the active experience of the user and provide a more lively feel to the program. However when computing a netlist larger than 20 connections, the user might have to wait a few minutes for a solution which might be an annoying experience. This is why I chose to limit the application to use database results, and not a solver. In the future, the application could be improved by collecting a larger database to give more value to the results.
